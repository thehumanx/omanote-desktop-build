...
function mapEvent(event: Doc<"eventEntries"> | Doc<"routineEntries">) {
  return {
    id: String(event._id),
    label: event.label,
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const backfillHashtagUsages = useMutation(api.hashtags.backfillHashtagUsages);
  ...
  useEffect(() => {
    ...
    void doSync();
    const interval = setInterval(() => void doSync(), 5 * 60 * 1000);
  ...
  const handleTodoAction = useCallback((action: AppAction): boolean => {
    switch (action.type) {
      ...
      case "todo/toggle": {
        ...
        const completedEventDateKey = isCompleting ? toDateKey(new Date()) : undefined;
        ...
        void (async () => {
          try {
            ...
            enqueueCanvasMutation("todo/toggle", { todoId: action.todoId });
          }
        })();
        return true;
      }
