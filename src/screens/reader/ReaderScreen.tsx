...
import { Button, Input, LoadingSpinner, Select, cn } from "../../components/ui";
...
// Enriched item type — raw rssItem + joined feed/read-state fields.
type ReaderItem = {
  _id: Id<"rssItems">;
  feedId: Id<"rssFeeds">;
  guid: string;
...
export function ReaderScreen({ savedView = false }: { savedView?: boolean }) {
  ...
  const savedItemsList = useLiveQuery(async () => {
    ...
    const itemIds = new Set(savedStates.map((rs) => String(rs.itemId)));
    const rawItems = await db.rssItems.filter((i) => itemIds.has(String(i._id))).toArray();
    const feedMap = new Map(
      (await db.rssSubscriptions.filter((s) => !s.deletedAt).toArray()).map((s) => [String(s.feedId), s])
    );
    const rsMap = new Map(savedStates.map((rs) => [String(rs.itemId), rs]));
    return rawItems
      .map((item): ReaderItem => {
        const sub = feedMap.get(String(item.feedId));
      ...
      .sort((a, b) => (rsMap.get(String(b._id))?.savedAt ?? 0) - (rsMap.get(String(a._id))?.savedAt ?? 0));
  ...
  const openItemFull = useQuery(
    api.rss.getItem,
    openItem ? { itemId: openItem._id } : "skip",
  );
  const openItemForModal = useMemo<ReaderItem | null>(() => {
    if (!openItem) return null;
    if (!openItemFull) return openItem;
    return {
      ...openItem,
      ...openItemFull,
    };
  }, [openItem, openItemFull]);
  ...
  const openArticle = (item: ReaderItem) => {
    ...
    if (!item.readAt) {
      ...
      void markRead({ itemId: item._id, read: true }).then(() => scheduleSync()).catch(() => {});
    }
  };
      ...
      {isMobileDrawer ? (
        ...
        <div className="mb-3">
          <SelectedFeedBar
            ...
            onMarkAllRead={() => {
              const now = Date.now();
              void db.rssReadState.where("feedId").equals(String(selectedFeedId!)).modify({ readAt: now, updatedAt: now });
                ...
                <>
                  ...
                  <button
                    ...
                    onClick={() => {
                      setDrawerFeedMenuOpen(false);
                      const now = Date.now();
                      void db.rssReadState.where("feedId").equals(String(selectedSubscription.feedId)).modify({ readAt: now, updatedAt: now });
                    ...
                    <>
                      ...
                      <button
                        ...
                        onClick={() => {
                          closeMenu();
                          const now = Date.now();
                          void db.rssReadState.where("feedId").equals(String(menuActions.feedId)).modify({ readAt: now, updatedAt: now });
    ...
    <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-app-line bg-app-surface px-4 py-2.5">
      ...
      >
        <span className="inline-flex items-center gap-1.5 text-[13px]">
          {markedRead ? <Check className="h-3.5 w-3.5" /> : <CheckCheck className="h-3.5 w-3.5" />}
          {markedRead ? "Done" : "Mark all read"}
        </span>
      </Button>
      <Button
        tone={confirming ? "danger" : "dangerGhost"}
        onClick={() => {
          if (!confirming) {
            setConfirming(true);
      ...
      >
        <span className="inline-flex items-center gap-1.5 text-[13px]">
          ...
          {confirming ? "Unsubscribe?" : "Unsubscribe"}
        </span>
      </Button>
    </div>
  );
        ...
        <div className="flex flex-wrap items-center gap-2 border-t border-app-line px-4 py-3 sm:px-6">
          <Button
            ...
            onClick={() => {
              ...
              void toggleSaved({ itemId: item._id, saved: next }).then(() => scheduleSync()).catch(() => setSaved(!next));
            }}
          >
            <span className="inline-flex items-center gap-1.5 text-[13px]">
          ...
          <Button
            ...
            onClick={() => {
              ...
              void markRead({ itemId: item._id, read: false }).then(() => scheduleSync()).catch(() => {});
              onClose();
            }}
          >
