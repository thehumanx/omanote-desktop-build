...
export function useHashtagPicker({
  value,
  textareaRef,
  onChange,
}: {
  ...
  // so the picker closes immediately when the cursor moves off a hashtag token.
  useEffect(() => {
  ...
  const active = useMemo(() => findActiveHashtag(value, cursorPos), [value, cursorPos]);
  ...
  const selectSuggestion = useCallback(
    (name: string) => {
      ...
      const newCursor = active.start + 1 + name.length + 1;
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLTextAreaElement | HTMLInputElement>): boolean => {
      ...
      if (event.key === "Enter" || event.key === "Tab") {
        ...
        if (selected) { event.preventDefault(); selectSuggestion(selected.name); return true; }
      }
      return false;
    },
...
export function HashtagPickerDropdown({
  isOpen,
  suggestions,
  activeIndex,
  onSelect,
    ...
    >
      <div className="p-1">
        {suggestions.length === 0 ? (
          <p className="px-3 py-2 text-xs text-app-ink-faint">Type to create a new hashtag</p>
        ) : (
          suggestions.map((hashtag, index) => (
            <button
              ...
              onMouseDown={(e) => { e.preventDefault(); onSelect(hashtag.name); }}
              className={[
