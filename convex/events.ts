...
const eventEntryIdValidator = v.union(v.id("eventEntries"), v.id("routineEntries"));
...
export const listEventEntries = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.dateKey) {
      ...
      return [...events, ...legacyEvents].sort((left, right) => right.loggedAt - left.loggedAt).slice(0, limit);
    }
    const [events, legacyEvents] = await Promise.all([
      ctx.db
      .query("eventEntries")
...
export const deleteEventEntry = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await removeCanvasArtifacts(ctx, { userId, artifactType: "routine", artifactId: String(args.eventId) });
    await removeArtifactHashtags(ctx, { userId, artifactType: "event", artifactId: String(args.eventId) });
    await removeArtifactHashtags(ctx, { userId, artifactType: "routine", artifactId: String(args.eventId) });
...
// One-time backfill: set updatedAt = createdAt for event/routine entries that predate the field.
...
// Incremental sync query — returns event entries (and legacy routine entries) updated after the given timestamp.
