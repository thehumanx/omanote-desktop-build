...
- **Instance-level cache**: results are cached in memory (1 h TTL, 500-entry cap) to avoid redundant fetches for the same URL within a warm Convex instance.

## Hashtag system

Hashtags are extracted from notes, todos, and event entries and stored in two tables:

- `userHashtags`: one row per unique hashtag name per user (the catalogue). Used for autocomplete and mindmap node list.
...
- `syncArtifactHashtags(ctx, { userId, artifactType, artifactId, artifactTitle, createdDateKey, createdAt, hashtags })` — upserts the catalogue and diffs the usage rows (deletes stale, inserts new, updates changed titles).
- `removeArtifactHashtags(ctx, { userId, artifactType, artifactId })` — deletes all usage rows for a deleted artifact.
