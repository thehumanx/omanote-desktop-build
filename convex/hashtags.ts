...
async function getActiveHashtagNames(ctx: QueryCtx, userId: string) {
  const usages = await ctx.db
    .query("hashtagUsages")
    .withIndex("by_user_hashtagName", (q) => q.eq("userId", userId))
    .take(MAX_HASHTAG_QUERY_LIMIT);
  return new Set(usages.map((usage) => usage.hashtagName));
}

async function filterHashtagsWithActiveUsage(ctx: QueryCtx, userId: string, hashtags: Doc<"userHashtags">[], limit: number) {
  const activeNames = await getActiveHashtagNames(ctx, userId);
  const active: Doc<"userHashtags">[] = [];
  for (const hashtag of hashtags) {
    if (activeNames.has(hashtag.nameLower)) {
      active.push(hashtag);
...
export async function syncArtifactHashtags(
  ...
  // 1. Upsert each hashtag name into the userHashtags catalogue (parallel reads)
  await Promise.all(hashtags.map(async (name) => {
    ...
    const existing = await ctx.db
      ...
      .unique();
    if (!existing) {
      await ctx.db.insert("userHashtags", {
        userId,
        name,
        nameLower,
        createdAt: timestamp,
  ...
  // 2. Fetch existing usages for this artifact
  ...
  // Delete stale usages, insert new ones, and update changed titles — all in parallel
  await Promise.all([
    ...existingUsages
      .filter((u) => !newSet.has(u.hashtagName))
  ...
  handler: async (ctx, args) => {
    ...
    if (prefixLower) {
      const hashtags = await ctx.db
        ...
        .take(MAX_HASHTAG_QUERY_LIMIT);
      return filterHashtagsWithActiveUsage(ctx, userId, hashtags, limit);
    }
    ...
    for (const item of args.items) {
      if (item.artifactType === "todo") {
        ...
        if (doc && doc.userId === userId) {
          ...
          const legacy = await ctx.db.get(item.artifactId as Id<"routineEntries">);
          if (!legacy || legacy.userId !== userId) continue;
          await ctx.db.patch(legacy._id, { hashtags: item.hashtags });
        }
      }
...
// ---------------------------------------------------------------------------
// Backfill – re-sync hashtagUsages from the hashtags already stored on each
// document. Safe to run multiple times (syncArtifactHashtags is idempotent).
// Needed for items that existed before the hashtag feature was deployed, or
// items whose hashtagUsages rows were cleared by the inline-edit bug.
// ---------------------------------------------------------------------------

export const backfillHashtagUsages = mutation({
  args: {},
  handler: async (ctx) => {
    const identity = await ctx.auth.getUserIdentity();
    const userId = requireUserId(identity);
    ...
    for await (const note of ctx.db
      .query("notes")
      .withIndex("by_user_createdAt", (q) => q.eq("userId", userId))) {
      if (!note.hashtags?.length || note.deletedAt) continue;
      await syncArtifactHashtags(ctx, {
        userId,
        artifactType: "note",
        artifactId: String(note._id),
    ...
    for await (const todo of ctx.db
      .query("todos")
      .withIndex("by_user_createdAt", (q) => q.eq("userId", userId))) {
      if (!todo.hashtags?.length || todo.deletedAt) continue;
      await syncArtifactHashtags(ctx, {
        userId,
        artifactType: "todo",
        artifactId: String(todo._id),
    ...
    for await (const event of ctx.db
      .query("eventEntries")
      .withIndex("by_user_createdAt", (q) => q.eq("userId", userId))) {
      if (!event.hashtags?.length || event.deletedAt) continue;
      await syncArtifactHashtags(ctx, {
        userId,
        artifactType: "event",
        artifactId: String(event._id),
        artifactTitle: event.label,
